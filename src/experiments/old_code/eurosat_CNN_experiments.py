...
if __name__ == "__main__":
    ...
    label_map = {'Residential': '0', 'AnnualCrop': '1', 'Highway': '2', 'SeaLake': '3', 'HerbaceousVegetation': '4',
                 'Pasture': '5', 'Industrial': '6', 'PermanentCrop': '7', 'Forest': '8', 'River': '9'}
    train_df['class'] = train_df['class'].map(label_map)
    validation_df['class'] = validation_df['class'].map(label_map)
    ...
    validation_df['class'] = validation_df['class'].astype('category')
    test_df['class'] = test_df['class'].astype('category')

    train_df = train_df.groupby('class').apply(lambda x: x.sample(200)).reset_index(drop=True).sample(frac=1).reset_index(drop=True)
    validation_df = validation_df.groupby('class').apply(lambda x: x.sample(200)).reset_index(drop=True).sample(frac=1).reset_index(drop=True)
    test_df = test_df.groupby('class').apply(lambda x: x.sample(200)).reset_index(drop=True).sample(frac=1).reset_index(drop=True)
