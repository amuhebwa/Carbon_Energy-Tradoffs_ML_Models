...
if __name__ == "__main__":
    ...
    train_df = pd.read_csv(f"{base_dir}/data/Kenya_Roads/train_df.csv")
    validation_df = pd.read_csv(f"{base_dir}/data/Kenya_Roads/validation_df.csv")
    test_df = pd.read_csv(f"{base_dir}/data/Kenya_Roads/test_df.csv")
    ...
    train_df.rename(columns={'ImageId': 'filename'}, inplace=True)
    validation_df.rename(columns={'ImageId': 'filename'}, inplace=True)
    test_df.rename(columns={'ImageId': 'filename'}, inplace=True)

    train_images = f"{base_dir}/data/Kenya_Roads/train"
    validation_images = f"{base_dir}/data/Kenya_Roads/validation"
    test_images = f"{base_dir}/data/Kenya_Roads/test"
    ...
    # convert IRI to classes
    train_df['class'] = train_df.apply(lambda row: roadsBinaryClassLabel(row['IRI']), axis=1)
    validation_df['class'] = validation_df.apply(lambda row: roadsBinaryClassLabel(row['IRI']), axis=1)
    test_df['class'] = test_df.apply(lambda row: roadsBinaryClassLabel(row['IRI']), axis=1)

    # convert class to string type(required for flow_from_dataframe)
