...
if __name__ == "__main__":
    ...
    """
    model_checkpoint = "facebook/convnext-large-224-22k-1k"
    ...
    if num_of_classes == 2:
        ...
        auroc = None
        f1 = f1_metric.compute(predictions=predicted_labels, references=actual_labels, average='weighted')
        precision = precision_metric.compute(predictions=predicted_labels, references=actual_labels, average='weighted')
        recall = recall_metric.compute(predictions=predicted_labels, references=actual_labels, average='weighted')
        print("AUROC is not applicable for multi-class classification : Fix this!!!!")


    accuracy = accuracy['accuracy']
    auroc = auroc['roc_auc'] if auroc is not None else None
